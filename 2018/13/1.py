...
while not found:
    ...
    for loc in curr_locs:
        ...
        if n_loc in new_locs:
            found = True
            print(str(n_loc[0]) + ',' + str(n_loc[1]))

        new_locs.append(n_loc)
        new_dirs.append(n_dir)
        new_counts.append(n_count)

    joined_locs = [(new_locs[i], new_dirs[i], new_counts[i]) for i in range(len(new_locs))]

    curr_locs = sorted(joined_locs, key=lambda x: (x[1], x[0]))
