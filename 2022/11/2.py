...
def round(monkey_list, primes_prod):
    ...
    for monkey in result_list:
        for item in monkey['items_holding']:
            ...
            if op == '*':
                worry_level = (f_hand * s_hand) % primes_prod
            elif op == '+':
                worry_level = (f_hand + s_hand) % primes_prod
            elif op == '-':
                worry_levle = (f_hand - s_hand) % primes_prod
            else:
                assert False
...
with open('input.txt', 'r') as f:
    ...
    primes_prod = math.prod(primes)

for i in range(0, 10000):
    result_dict = round(monkeys, primes_prod)
