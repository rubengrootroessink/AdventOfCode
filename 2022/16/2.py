...
for choice in solutions(dist, flow_rates, pressurized_valves, 26):
    path = frozenset(choice)
    flow = eval_path(flow_rates, choice)
    if flow > max_scores[path]:
        max_scores[path] = flow

max_flow = 0
for (s1, score1), (s2, score2) in combinations(max_scores.items(), 2):
    if s1 & s2:
        continue
    max_flow = max(score1 + score2, max_flow)
